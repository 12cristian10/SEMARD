...
uint32_t readingInterval = 750, initialT = 0;  // Intervalo de lectura de sensores en milisegundos y tiempo inicial
...
void loop() {
  ...
  if (currentT - initialT >= readingInterval) {  // Verificar si ha pasado el intervalo de lectura deseado
    ...
    phVoltage = ads.computeVolts(phValues)*1000;
    ecVoltage = ads.computeVolts(ecValues)*1000;
    odVoltage = ads.computeVolts(odValues)*1000;
    tdsVoltage = ads.computeVolts(tdsValues);
